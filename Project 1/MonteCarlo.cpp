...
int main( int argc, char *argv[ ] ) {
	...
	dataFile.open("monteCarloData.csv", std::ios_base::app);
	...
	for( int tries = 0; tries < NUMTRIES; tries++ )
	{
		...
		#pragma omp parallel for default(none), shared(vs), shared(ths), shared(gs), shared(hs), shared(ds), shared(stderr), reduction(+:numHits)
		for( int n = 0; n < NUMTRIALS; n++ )
		{
	...
	dataFile << NUMT << "," << NUMTRIALS << "," << probability << "," << maxPerformance << "\n";
